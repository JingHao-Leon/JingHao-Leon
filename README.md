...
### 🏭 [fde-case-lab](https://github.com/JingHao-Leon/fde-case-lab)
真实 FDE 案例的可运行工程实现集：把《Datawhale FDE 案例 100》中 **16 个企业落地场景**做成「求解器 + 测试 + 回测」——装车 3D 装箱首车 **97.1 方**、对账净额还原误差 **-93.8%**、补货×广告联动总成本 **-67.5%**（UCI 真实零售需求流回测）、非标报价 MAPE **58.4%→9.2%**、确定性分析 Agent 稳定性 **100%**。**106 个测试全绿**，数据来源与许可全公开，所有数字一条命令可复现。
